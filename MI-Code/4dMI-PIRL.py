...
def setup_logging():
    ...
    log_filename = '4D_MI_PIRL_training_{}.log'.format(timestamp)
...
def menet_pricer_nd(t0, S0, K, T, r, sigma, rho, num_paths, num_timesteps):
    ...
    with torch.no_grad():
        ...
        if t0.dim() == 0:  
            ...
            dt = (T - t0.unsqueeze(1)) / num_timesteps
        
        S = S0.unsqueeze(1).expand(-1, num_paths, -1)
...
def create_4d_comparison_plots(pirl_model, mi_pirl_model, n, sigma, rho, r, T, K):
    """Create comparison plots for 4D case with both PIRL and MI-PIRL models"""
 
    logger.info("\n-- Creating 4D Comparison Plots --")
    ...
    S_fixed = [[0.95, 1.05, 0.90, 1.00], [1.10, 1.00, 1.15, 0.95]]
    colors = ['blue', 'red']
    ...
    plt.title('PIRL vs MI-PIRL vs MC: Option Price vs Time (4D)')
    plt.legend(fontsize=8)
    plt.grid(True, alpha=0.3)
    
    plt.tight_layout()
    plt.savefig('4D_MI_PIRL_price_comparison.png', dpi=300)
...
def main():
    """Main function to run 4D MI-PIRL comparison"""
    ...
    shared_params = {
        ...
        'use_scheduler': True,
        'lambda_bc': 1.0,
        'lambda_martingale': 10.0,
        
        'N_interior': 2000 * n,
        'N_bc': 400 * n,
        'N_martingale': 200 * n,
        
        'menet_paths_training': 8000,
    ...
    rho = np.array([[1.0, 0.5, 0.3, 0.2],
                    [0.5, 1.0, 0.4, 0.3],
                    [0.3, 0.4, 1.0, 0.5],
                    [0.2, 0.3, 0.5, 1.0]])
    
    logger.info("Starting 4D MI-PIRL Training and Evaluation (Ultra-Optimized)")
    ...
    logger.info("="*80)
    logger.info("4D MI-PIRL vs 4D PIRL COMPARISON (ULTRA-OPTIMIZED)")
    logger.info("="*80)
    ...
    try:
        ...
        mae_pirl, mae_mi_pirl, mse_pirl, mse_mi_pirl = create_4d_comparison_plots(
        ...
        plt.yscale('log')
        plt.title('Loss Convergence Comparison (4D Min-Call Option - Ultra-Optimized)')
        plt.xlabel('Epoch')
        ...
        plt.tight_layout()
        plt.savefig('4D_MI_PIRL_loss_comparison_ultra_optimized.png', dpi=300)
        logger.info("Loss comparison figure saved as: 4D_MI_PIRL_loss_comparison_ultra_optimized.png")
